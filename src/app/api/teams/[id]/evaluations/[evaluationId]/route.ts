...
export async function DELETE(_req: Request, { params }: { params: { id: string; evaluationId: string } }) {
  const session = await getServerSession(authOptions);
  if (!session?.user) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });

  const evaluation = await prisma.evaluation.findUnique({ where: { id: params.evaluationId } });
  if (!evaluation || evaluation.coachId !== session.user.id || evaluation.teamId !== params.id) {
