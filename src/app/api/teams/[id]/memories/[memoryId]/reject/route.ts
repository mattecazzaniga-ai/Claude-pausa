...
import { rejectTeamMemory } from "@/lib/memory";
...
export async function POST(req: Request, props: { params: Promise<{ id: string; memoryId: string }> }) {
  ...
  await rejectTeamMemory(params.memoryId, session.user.id);
  track("team_memory_rejected", session.user.id, { teamId: params.id, memoryId: params.memoryId });
