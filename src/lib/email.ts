...
const fromAddress = process.env.EMAIL_FROM || "CoachBrain <onboarding@resend.dev>";
...
export async function sendPasswordResetEmail(to: string, rawToken: string) {
  ...
  await sendEmail(
    to,
    "Reimposta la tua password CoachBrain",
    `<p>Hai richiesto di reimpostare la password del tuo account CoachBrain.</p>
